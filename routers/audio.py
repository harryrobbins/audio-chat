...
async def generate_audio(script_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # 3. Setup British English Kokoro Pipeline
    # British language code is 'b'
    print("Loading Kokoro-82M pipeline for British English (lang='b')...")
    ...
    print(f"Generating audio for script {script_id} with {len(lines)} lines...")
    ...
    for i, line in enumerate(lines):
        ...
        try:
            ...
            print(f"Error synthesizing line {i+1}: {e}")
            raise HTTPException(
                status_code=500, 
                detail=f"TTS generation failed at line {i+1}: {str(e)}"
            )

    # 5. Export final concatenated audio to disk
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to export concatenated WAV file: {str(e)}"
        )
